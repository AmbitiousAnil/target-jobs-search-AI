...
class LLMProviderService(ABC):
    ...
    def canonical_name(cls) -> str:
        if not cls.provider_names:
            raise ValueError(f"{cls.__name__} must define provider_names.")
        return cls.provider_names[0]
...
class LiteLlmAdkService(LLMProviderService):
    ...
    def _load_litellm_class(self) -> Any:
        ...
        except Exception as exc:  # pragma: no cover - only exercised in ADK runtime
            raise RuntimeError(
                f"{self.provider_label} support requires google-adk to be installed in the runtime."
            ) from exc
        return LiteLlm
