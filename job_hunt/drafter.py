...
from autopilot_jobhunt.services.pdf_utils import write_text_pdf
...
def _resolve_job(job_ref: str) -> tuple[str, str]:
    ...
    job = jobs[idx]
    logger.debug(
        f"Resolved job #{idx + 1}: "
        f"'{job.get('extracted_title') or job.get('title')}' @ "
        f"{job.get('company')} - {job.get('url')}"
    )
...
def draft_application(
    config: dict,
    job_ref: str,
    tailoring_guidance: dict[str, str] | None = None,
) -> None:
    ...
    resume_guidance = _guidance_block(
        "TAILORING SKILL GUIDANCE FOR RESUME REWRITES",
        tailoring_guidance.get("resume_guidance"),
    )
    cover_guidance = _guidance_block(
        "TAILORING SKILL GUIDANCE FOR COVER LETTERS",
        tailoring_guidance.get("cover_letter_guidance"),
    )
    checklist_guidance = _guidance_block(
        "APPLICATION OUTPUT CHECKLIST",
        tailoring_guidance.get("application_checklist"),
    )
...
Output ONLY the tailored resume in Markdown. No preamble."""}],
    ...
    tailored_resume_pdf_path = out_dir / f"resume_{company_slug}.pdf"
    write_text_pdf(
        resume_md,
        tailored_resume_pdf_path,
        title=f"Tailored Resume - {candidate_name}",
        subtitle=f"Target job: {job_url}",
    )
    logger.info(f"  Saved: {tailored_resume_pdf_path}")
...
Output ONLY the cover letter. No preamble."""}],
    ...
    cover_pdf_path = out_dir / f"cover_letter_{company_slug}.pdf"
    write_text_pdf(
        cover_md,
        cover_pdf_path,
    ...
    logger.info(f"  Saved: {cover_pdf_path}")
