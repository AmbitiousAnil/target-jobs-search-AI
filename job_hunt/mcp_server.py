...
def scan_jobs() -> str:
    ...
    score them with AI against your resume, and write the top matches to CSV.
    Reads config.json and companies.json from the working directory.
    """
    return tool_scan()
...
def draft_application(job_ref: str) -> str:
    ...
    Returns a summary of where the Markdown and PDF output files were saved.
    """
    return tool_draft(job_ref)


@mcp.tool()
def export_jobs(min_score: int = 0, days: int = 0, export_format: str = "csv") -> str:
    """
    Export job scan results to a CSV file, a PDF file, or both in the output/ directory.

    Args:
        min_score: Only include jobs with score >= this value (0 = all).
        days: Export from the last N days of history (0 = last scan only).
        export_format: One of csv, pdf, or both.

    Returns a summary of the export.
    """
    return tool_export(min_score=min_score, days=days, export_format=export_format)
