...
def extract_state(tool_context: Any | None) -> MutableMapping[str, Any] | None:
    if tool_context is None:
        return None
    for candidate in (
        getattr(tool_context, "state", None),
        getattr(getattr(tool_context, "session", None), "state", None),
        getattr(getattr(getattr(tool_context, "invocation_context", None), "session", None), "state", None),
    ):
...
def _peek_session_id(tool_context: Any | None) -> str | None:
    ...
    state = extract_state(tool_context)
    if state is not None and state.get("jobhunt_session_id"):
        return str(state["jobhunt_session_id"])
    return None


def session_dir_from_tool_context(tool_context: Any | None) -> Path | None:
    state = extract_state(tool_context)
...
def _load_disk_state(tool_context: Any | None) -> dict[str, Any] | None:
    session_dir = session_dir_from_tool_context(tool_context)
...
def _write_disk_state(session_dir: Path, payload: dict[str, Any]) -> None:
    _disk_state_path_from_session_dir(session_dir).write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def persist_session_configuration(tool_context: Any | None, config: JobSearchConfiguration, staged: StagedSession, *, rescan_required: bool = True, config_change_summary: str | None = None) -> None:
    payload = {
        "jobhunt_session_id": staged.session_id,
        CONFIG_STATE_KEY: config.public_summary() | {"session_dir": str(staged.session_dir), "rescan_required": rescan_required},
        INTERNAL_CONFIG_STATE_KEY: asdict(config),
        SUMMARY_STATE_KEY: f"roles={', '.join(config.target_roles)} | locations={', '.join(config.target_locations)} | companies={len(config.company_urls)} | min_score={config.min_score} | top_n={config.top_n}",
...
def _load_state_value(tool_context: Any | None, key: str):
    state = extract_state(tool_context)
    if state is not None and key in state:
        return state.get(key)
    disk_state = _load_disk_state(tool_context)
    return disk_state.get(key) if disk_state else None


def load_session_configuration(tool_context: Any | None) -> dict | None:
    value = _load_state_value(tool_context, CONFIG_STATE_KEY)
    return value if isinstance(value, dict) else None


def load_internal_session_configuration(tool_context: Any | None) -> dict | None:
    value = _load_state_value(tool_context, INTERNAL_CONFIG_STATE_KEY)
    return value if isinstance(value, dict) else None


def load_rescan_required(tool_context: Any | None) -> bool:
    value = _load_state_value(tool_context, RESCAN_REQUIRED_STATE_KEY)
    return bool(True if value is None else value)


def load_config_change_summary(tool_context: Any | None) -> str | None:
    value = _load_state_value(tool_context, CONFIG_CHANGE_SUMMARY_STATE_KEY)
    return value if isinstance(value, str) and value.strip() else None


def update_rescan_state(tool_context: Any | None, *, rescan_required: bool, config_change_summary: str | None = None) -> None:
    state = extract_state(tool_context)
    if state is not None:
