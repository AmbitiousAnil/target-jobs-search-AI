...
def _missing_dependency_error(packages: str, use_case: str) -> RuntimeError:
    return RuntimeError(
        f"{use_case} requires {packages}. Install the project dependencies again so PDF support is available."
    )


def _normalize_text(value: str) -> str:
    text = str(value or "").translate(_PDF_TEXT_TRANSLATION).replace("\r\n", "\n").replace("\r", "\n")
    text = re.sub(r"\n{3,}", "\n\n", text)
    return text.strip()
...
def extract_text_from_pdf_bytes(data: bytes) -> str:
    ...
    if not text:
        raise RuntimeError("Could not extract text from the uploaded resume PDF.")
    return text
...
def _escape_paragraph_text(text: str) -> str:
    escaped = html.escape(_normalize_text(text))
    ...
    *,
    ...
    def flush_bullets() -> None:
        ...
        if not bullet_buffer:
            return
        items = [
            list_item(paragraph(_escape_paragraph_text(item), styles["compact"]))
...
def write_jobs_pdf(
    ...
    *,
    ...
    for job in jobs:
        ...
        location = str(job.get("location_remote") or job.get("location") or "").strip()
        url = str(job.get("url") or "").strip()
        reason = str(job.get("reason") or "").strip()
        rows.append(
            [
                paragraph(_escape_paragraph_text(score), styles["compact"]),
                paragraph(
                    f"{html.escape(_normalize_text(company))}<br/>{html.escape(_normalize_text(role))}",
                    styles["compact"],
                ),
                paragraph(_escape_paragraph_text(location or "-"), styles["compact"]),
                paragraph(_escape_paragraph_text(url or "-"), styles["compact"]),
                paragraph(_escape_paragraph_text(reason or "-"), styles["compact"]),
            ]
        )
    ...
    story.append(
        paragraph(
            _escape_paragraph_text(
                f"Included {len(jobs)} jobs with score >= {min_score}."
            ),
            styles["subtitle"],
        )
    )
...
def artifact_inline_bytes(artifact: Any) -> bytes:
    ...
    if inline_data is None or getattr(inline_data, "data", None) is None:
        raise RuntimeError("The uploaded artifact did not include inline PDF data.")
...
async def save_file_artifact(
    ...
    *,
    ...
    except ImportError as exc:  # pragma: no cover - depends on ADK runtime
